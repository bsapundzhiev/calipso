...
static int calipso_request_read_header(calipso_request_t *request)
{
	...
	for(;;) {
		...
		if ( (INPUTBUFSZ-1) <= c->size) {
			*(c->b + c->size) = '\0';
			return calipso_reply_set_status(request->reply, HTTP_REQUEST_URI_TOO_LARGE);
		}
		...
		if(n <= 0) {
			if(errno == EAGAIN) {
				...
				*(c->b + c->size) = '\0';
				cpo_log_error(calipso->log, "connection error %s", strerror(errno) );
			...
			if(p < 0x9 || p > 0x7e) {
				*(c->b + c->size) = '\0';
				...
				return calipso_reply_set_status(request->reply, HTTP_BAD_REQUEST);
			}
...
int
calipso_request_set_config(calipso_request_t *request, calipso_config_t *config)
{
...
int
calipso_request_set_time(calipso_request_t *request, time_t request_time)
{
...
static int request_set_header_vals(calipso_request_t *request)
{
	...
	if(host) {
		char * p = strrchr(host, ':');
		request->host = (p) ? cpo_pool_strndup(request->pool, host, p - host):
		cpo_pool_strdup(request->pool, host);
	}
...
int
calipso_request_parse_header_buf(calipso_request_t *request)
{
...
static int
request_parse_status_line(calipso_request_t *request, char *line)
{
    char *method = NULL;
    char *uri = NULL;
    char *lastword = NULL;
...
/* parser header lines */
static int
request_parse_header(calipso_request_t *request, char *header)
{
    char *eol;
    char *hdr;
    char *val;

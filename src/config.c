...
enum {
    PERR_LINE, PERR_DELIM, PERR_PAIR, PERR_BEGIN, PERR_END, PERR_BLOCK,
};
...
void config_set_option(calipso_config_t *config, const char *opt,
                       const char *val, const char *section)
{
...
static void config_parse_line(calipso_config_t *config, char *line, int nline)
{
    ...
    if (block_begin(line)) {
        ...
        if (!strlen(_block)) {
            ...
            if (s) {
                ...
                parser_error(nline, PERR_BLOCK);
        } else
            parser_error(nline, PERR_BEGIN);
        //printf("begin '%s'\n", _block);
        return;
    }
    ...
    if (strlen(option) && strlen(value)) {
        trim(option);
        trim(value);
        config_set_option(config, option, value,
                          strlen(_block) ? _block : NULL);
    } else {
...
const char *config_get_option(calipso_config_t *config, const char *opt,
                              const char *section)
{
    conf_ctx_t *cfg;
...
int config_get_noption(calipso_config_t *config, const char *opt,
                       const char *section)
{
...
double config_get_foption(calipso_config_t *config, const char *opt,
                          const char *section)
{
    const char *val = config_get_option(config, opt, section);
...
int calipso_config_set_parent(calipso_config_t * parent,

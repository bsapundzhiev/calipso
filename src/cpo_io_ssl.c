...
ssize_t cpo_io_ssl_read(calipso_client_t *client, void *buf, size_t len)
{
    return fd_ssl_read(client->ssl, buf, len);
}
ssize_t cpo_io_ssl_write(calipso_client_t *client, const void *buf, size_t len)
{
    ...
    if (client->ssl && SSL_in_init(client->ssl)) {
        ...
        if (ret < 0) {
            ...
            default:
                ...
                cpo_log_error(calipso->log, "connection error SSL_error(%d)",
                              err);
...
ssize_t fd_ssl_write(SSL *ssl, const void *buf, size_t len)
{
    int cc;
    int total;
    total = 0;
...
ssize_t fd_ssl_read(SSL *ssl, void *buf, size_t len)
{
    int cc;
    int total;
    total = 0;

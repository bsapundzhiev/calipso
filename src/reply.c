...
size_t
calipso_reply_get_replybuf_size(calipso_reply_t *reply)
{
    return (reply->replybufsz);
}

int
calipso_reply_set_replybuf_size(calipso_reply_t *reply, size_t replybufsz)
{
...
int
calipso_reply_set_state(calipso_reply_t *reply, int state)
{
    return ((reply->state = state));
}

int
calipso_reply_get_state(calipso_reply_t *reply)
{
    return (reply->state);
}
...
int
calipso_reply_set_config(calipso_reply_t *reply, calipso_config_t *config)
{
...
int 
calipso_reply_send_writev(calipso_reply_t *reply)
{
...
int
calipso_reply_send_write(calipso_reply_t *reply)
{
    int ret, read;
    size_t replybufsz;
	...
	calipso_reply_set_replybuf_size(reply, reply->out_filter->total_bytes);
	TRACE("bytes_to_send %d bytes_sent %d read %d\n", reply->bytes_to_send, reply->bytes_sent , read );
...
#else
	nr = sendfile( clientsock, rdesc, &(reply->resource)->offset, calipso_resource_get_size(reply->resource) );
...
#endif
	TRACE(" nr %d ? %lld\n", nr, calipso_resource_get_size(reply->resource));
	}
	if(nr <= 0)
	{
		if(nr == -1 && errno == EAGAIN) return 0;

		if(nr < 0) perror("Sendfile: ");
		close(rdesc);

...
static void calipso_unalloc_listeners(hash_t *listeners)
{
	u_int i;
...
static int calipso_load_log_default(calipso_t * calipso)
{
	...
	if(calipso->config) {
		log->file_access = (char*)config_get_option(calipso->config, "acces_log" , NULL);	
		log->file_error = (char*)config_get_option(calipso->config, "error_log" , NULL);	
	}
...
int calipso_set_pool(calipso_pool_t *pool)
{
    return ((calipso->pool = pool) != NULL);
}

calipso_pool_t * calipso_get_pool(void)
{
    return (calipso->pool);
}

int calipso_set_config(calipso_config_t *config)
{
    return ((calipso->config = config) != NULL);
}

calipso_config_t * calipso_get_config(void)
{

    return (calipso->config);
}

int calipso_set_listeners_hash(hash_t *hash)
{
    return ((calipso->listeners = hash) != NULL);
}

hash_t * calipso_get_listeners_hash(void)
{
    return (calipso->listeners);
}

List * calipso_get_listeners_list() 
{
	u_int i;
	hash_node_t *node;
...
int calipso_add_listener(calipso_socket_t *listener)
{
    char key[6];
    hash_t * hash_entry;
...
List * calipso_get_m_hook(int hook)
{
    return calipso->m_hook[hook];
}

int calipso_set_handler_hash(hash_t *hash)
{
	return ((calipso->m_handler = hash) != NULL);
}

hash_t * calipso_get_handler_hash(void)
{
...
calipso_mod_t * calipso_get_module_table(void)
{
...
int calipso_set_current_process(calipso_process_t *process)
{
    return ((calipso->currentproc = process) != NULL);
}

calipso_process_t * calipso_get_current_process(void)
{
    return (calipso->currentproc);
}

int calipso_set_mprocess(calipso_process_t *process)
{
    return ((calipso->mprocess = process) != NULL);
}


int calipso_set_mprocess_model(int (*mproc_model)(void))
{
...
int calipso_set_nprocess_model(int (*nproc_model)(void))
{
...
int calipso_set_pw(void *pw)
{
    return ((calipso->pw = pw) != NULL);
}

int calipso_set_gr(void *gr)
{
    return ((calipso->gr = gr) != NULL);
}

int calipso_set_uid(uid_t uid)
{
    return ((calipso->uid = uid));
}

int calipso_set_gid(gid_t gid )
{
...
char *calipso_get_name(void)
{
    return ("Calipso");
}

char *calipso_get_version(void)
{
    return (VERSION);
}

cpo_log_t *calipso_get_log()
{
	return calipso->log;
}

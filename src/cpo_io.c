...
ssize_t
cpo_io_read(calipso_client_t * client, void *buf, size_t len)
{
	return fd_read(client->csocket, buf, len);
}

ssize_t
cpo_io_write(calipso_client_t * client, const void *buf, size_t len)
{
...
ssize_t splice_sendfile(int infd, int outfd, off_t *offset, ssize_t size)
{
	...
	ret = splice(infd, offset, p[1], NULL, size, SPLICE_F_MOVE | SPLICE_F_NONBLOCK);
	if (ret < 0) {
		total = ret;
		goto out;
   	}
	...
	while (total < ret) {
    	written = splice(p[0], NULL, outfd, NULL, ret, SPLICE_F_MORE | SPLICE_F_MOVE | SPLICE_F_NONBLOCK);
		if(written < 0) {
			perror("splice()");

...
int calipso_socket_get_socket(calipso_socket_t *listener)
{
...
int16_t calipso_socket_get_port(calipso_socket_t *socket)
{
    return (socket->port);
}
...
calipso_socket_accept_client(calipso_socket_t *listener)
{
    ...
    client->csocket = accept(listener->lsocket,
                             (struct sockaddr *) client->info, &len);
...
int calipso_socket_set_read_handler(calipso_socket_t *listener,
...
int calipso_socket_set_write_handler(calipso_socket_t *listener,
...
int calipso_socket_set_accept_callback(
    calipso_socket_t *listener,
    int (*accept_callback)(calipso_client_t *))
{
    return ((listener->accept_callback =
                 (void *) accept_callback) != NULL);
}

int calipso_socket_add_server(calipso_socket_t *listener,
                              calipso_server_t *server)
{
...
static int socket_create_inet(const char *addr, int16_t portn)
{
    ...
    sin.sin_addr.s_addr =
        (addr == NULL) ?
        htonl(INADDR_ANY) : inet_addr(addr);

    ret = bind(lsocket, (struct sockaddr*) &sin,
...
char * calipso_socket_get_peer_name(int s)
{
    ...
    if (getpeername(s, (struct sockaddr *) &sa,
                    (socklen_t *) &sa_len) == -1) {
        perror("getpeername(): ");
        return NULL;
    }

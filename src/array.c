...
static int
cpo_array_preallocate(cpo_array_t *a, int elements)
{
	void *newv;
	int newmax = a->max;
...
void cpo_array_qsort(cpo_array_t *a, int (*cmp_func)(const void *, const void *))
{
	qsort(a->v, a->num, a->elem_size, cmp_func);
}

void *cpo_array_bsearch(cpo_array_t *ar,const void *key,int (*compar)(const void *, const void *)) 
{

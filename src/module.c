...
static void calipso_register_module(calipso_mod_t * m, char *module_name,
				    char *path);
...
static void calipso_win32_intializer(void *module_hdl)
{
	...
	pt2f init = Sys_LoadFunction(module_hdl, "CalipsoModWin32Initializer");
	if (!init) {
					...
					char const *function_name)
{
	...
	if (function_name == NULL) {
		...
		case HOOK_INIT:
			p_addr =
			    Sys_LoadFunction(module_hdl, MODULE_INIT_FUNCTION);
			break;
		case HOOK_END:
			p_addr =
			    Sys_LoadFunction(module_hdl, MODULE_EXIT_FUNCTION);
			break;
		case HOOK_CONFIGURE:
			p_addr =
			    Sys_LoadFunction(module_hdl,
					     MODULE_CONFIGURE_FUNCTION);
			break;
		default:
			TRACE("Bad hook");
			return 0;
				    ...
				    char *path)
{
	...
	if (!m->handler) {
		fprintf(stderr, "%s():%d dlerror= %s - terminate\n", 
						__func__, __LINE__, Sys_LibraryError());
		exit(-1);
	}
	...
	for(config = list_get_first_entry(config); 
		...
		if (conf && strcmp(conf->option, "load_module") == 0) {
			...
			if (MAX_LOADED_MODULES > nr_modules) {
				calipso_register_module(&module_table
							[nr_modules], name,
							path);
				nr_modules++;
			} else {
